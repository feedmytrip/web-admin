    ...
    <div class="card-content" v-if="!loading">
      <div class="content">
        ...
        <div class="field">
          ...
          <div class="control">
            <input class="input is-small" type="text" placeholder="PT" v-model="category.title.pt">
          </div>
        </div>
        <div class="field">
          <label class="label is-small">Title ES</label>
          <div class="control">
            <input class="input is-small" type="text" placeholder="PT" v-model="category.title.es">
          </div>
        </div>
        <div class="field">
          <label class="label is-small">Title EN</label>
          <div class="control">
            <input class="input is-small" type="text" placeholder="PT" v-model="category.title.en">
          </div>
        </div>
      </div>
    </div>
...
<script>
  export default {
    props: ['itemId', 'data'],
    created() {
      const index = _.findIndex(this.data, {'categoryId': this.itemId})
    ...
    methods: {
      update() {
        this.loading = true
        this.$store.dispatch('auxiliary/updateCategory', this.category)
        .then( result => {
          this.loading = false
          this.$parent.close()
        })
